...
public class Control extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      try {
         ...
         buffer.append("<div class=mnuprof>'.$_mess24.'</div>");
         buffer.append("</th>");
         buffer.append("</tr><tr>");
         ...
         buffer.append("<table class=control><tr class=heads>");
         buffer.append("<th class=internal>");
         buffer.append("<div class=mnuprof>'.$_mess23.'</div>");
         buffer.append("</th>");
         buffer.append("</tr><tr>");
         buffer.append("<td class=internal>");
         ...
         if (id>1 && id<6){
            ...
            String $textarea="";
            if (error != null) {
               $textarea = (String) session.getAttribute("body");
            }
            buffer.append("<textarea class='mnuforumSm' rows='20' id='ed1' name='A2' cols='55'>" + $textarea + "</textarea>");
            buffer.append("</p>");
            /*Кнопки*/
            buffer.append("<table>");
            buffer.append("<tr>");
   ...
   private StringBuffer case1(User user, LocaleString locale) throws InvalidKeyException, IOException{
      ...
      if (ignorList.size() == 0) {
         ...
         buffer.append("</th>");
         buffer.append("</tr>");
         for (int $x1=0; $x1<ignorList.size(); $x1++) {
            Ignor ignor = ignorList.get($x1);
            // id Записи
            Long $str_id = ignor.getId();
            // Тип игнора
            Integer $str_type= ignor.getType();
            // Игнорируемый
            String $str_nick = ignor.getUser().getNick();
            // Начал
            Date $str_begin = ignor.getStart(); 
            // Конец
            Date $str_end = ignor.getEnd();
            // Выводим спи
            buffer.append("<tr>");
            // Номер
            buffer.append("<td class='internal'>");
            buffer.append("<span class=mnuprof>" + ($x1+1) + "</span>");
            buffer.append("</td>");
            // Игнорируемый
            buffer.append("<td class='internal'>");
            buffer.append("<span class=mnuprof>" + $str_nick + "</span>");
            buffer.append("</td>");
            // Начал
            buffer.append("<td class='internal'>");
            buffer.append("<span class=mnuprof>" + $str_begin + "</span>");
            buffer.append("</td>");
            // Конец
            buffer.append("<td class='internal'>");
            buffer.append("<span class=mnuprof>" + $str_end + "</span>");
            buffer.append("</td>");
            // Ветки игнорируются?
            buffer.append("<td class='internal' align='CENTER'>");
            String $str_tema=locale.getString("mess26");
            if ($str_type == 0) $str_tema=locale.getString("mess27");
            buffer.append("<span class=mnuprof>" + $str_tema + "</span>");
            buffer.append("</td>");
            // Изменение
            buffer.append("<td class='internal'>");
            ...
            buffer.append("<option selected value='01'><span class=mnuprof>$_mess32</span></option>");
            ...
            // id Записи
            buffer.append("<input type=hidden name=\"IDZ\" size=\"20\" value=\"" + $str_id + "\">");
            // Автор
            buffer.append(fd_form_add(user));
            buffer.append("</form>");
            buffer.append("</td>");
